...
# 
file_index <- 9
file_path <- paste0("Data/gameplay_data_V/", txt_files[file_index])
...
dygraph(pause.ts, xlab = "Keypress index", ylab = "Pause length", 
        main = paste0("V ", file_index, " ", txt_files[file_index], " ", obs)) %>%
...
dygraph(lowPass_filter, xlab = "Keypress index", ylab = "Pause length", 
        main = paste0("V ", file_index, " : low pass filter ", obs)) %>%
  dySeries("V1", label = "Seconds") %>%
  dyLegend(show = "always", hideOnMouseOut = FALSE) %>%
  dyRangeSelector()
