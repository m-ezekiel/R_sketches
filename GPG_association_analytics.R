...
buttons <- data.frame(blue = as.logical(A1), green = as.logical(A2), red = as.logical(A3), opacity = as.logical(A4), disp = as.logical(L1), size = as.logical(R1), random = as.logical(L2), mute = as.logical(R2), select1 = as.logical(S1), select2 = as.logical(S2), up = as.logical(up), down = as.logical(down), left = as.logical(left), right = as.logical(right))
...
# frequent 2-itemsets
itemsets2 <- apriori(trans, parameter = list(minlen = 2, maxlen = 2, support = 0.002, target="frequent itemsets"))
summary(itemsets2)
